...
RK-local C++17 API 通过设备本机 `/run/prism/stream.sock` 访问 Agent，不直接读 UART。
完整的 Host / Windows / RK-local 接口对应表见 [英文接口说明](gnss-rtk.md)。

## GNSS/PPS 状态

Host 和 RK-local 都使用 `gnssTimingStatus()`，返回同一 `prism::GnssTimingStatus`。
...
Host/RK-local 的 `rtkCorrectionStatus()` 上报检测到的
...
Host/RK-local 的 `rtkNavigationStatus()` 同时报告两套数据。
...
RK-local 的 `readRtkNavigation()` 等待最新事件；慢消费者只保留
最新未读快照，不应阻塞采集。
